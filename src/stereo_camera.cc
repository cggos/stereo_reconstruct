...
void StereoCamera::compute_disparity_map(const cv::Mat &mat_l, const cv::Mat &mat_r, cv::Mat &mat_disp, int method) {
...
void StereoCamera::depth_to_pointcloud(const cv::Mat &mat_depth,
...
void StereoCamera::get_colormap_ocv(const cv::Mat &mat_in, cv::Mat &color_map, cv::ColormapTypes colortype) {
  ...
  cv::Mat mat_scaled;
  if (min != max) mat_in.convertTo(mat_scaled, CV_8UC1, 255.0 / (max - min), 0);  // -255.0 * min / (max - min)
